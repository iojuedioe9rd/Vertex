...
namespace Vertex {
	...
	namespace Utils {
		...
		static GLenum VertexFBTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			VX_CORE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			VX_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	{
		VX_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::VertexFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
