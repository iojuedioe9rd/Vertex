...
namespace Vertex {
	...
	struct VERTEX_API ProfileResult
	{
		std::string Name;
	...
	struct VERTEX_API InstrumentationSession
	{
		std::string Name;
	};

	class VERTEX_API Instrumentor
	{
	public:
	...
	class VERTEX_API InstrumentationTimer
	{
	public:
	...
	namespace InstrumentorUtils {
		...
		template <size_t N>
		struct VERTEX_API ChangeResult
		{
			char Data[N];
		};

		template <size_t N, size_t K>
		constexpr VERTEX_API auto CleanupOutputString(const char(&expr)[N], const char(&remove)[K])
		{
...
#define VX_PROFILE_BEGIN_SESSION(name, filepath) ::Vertex::Instrumentor::Get().BeginSession(name, filepath)
#define VX_PROFILE_END_SESSION() ::Vertex::Instrumentor::Get().EndSession()
#define VX_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Vertex::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Vertex::InstrumentationTimer timer##line(fixedName##line.Data)
