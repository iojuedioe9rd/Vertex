...
namespace Vertex
{
	...
	public:
		...
		static Ref<Shader> Create(const std::string& filepath, const bool isHLSL = false);
		static Ref<Shader> Create(const std::string& name, const std::string& vertexSrc, const std::string& fragmentSrc, const bool isHLSL = false);
	private:
		
	};

	class VERTEX_API ShaderLibrary
	{
	public:
